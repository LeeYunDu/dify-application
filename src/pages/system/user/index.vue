...
import { getUserListApi,changeUserStatusApi,delUserApi } from '@/api/modules/java.system.user'
  ...
  const actionMap:any = {
    ...
    'delete':async () => {
      ElMessageBox.confirm(
        ...
        .then(async () => {
          const { status,msg } = await delUserApi(row.userId)
          if (!(status===200)) return ElMessage.error(msg)
          ElMessage.success('操作成功')
          asyncData()
        })
...
const asyncData = async () => {
  ...
  const { status,msg,data,total,rows } = await getUserListApi(params)
  if (!(status===200)) return ElMessage.error(msg)
  tableOptions.options.pagination.total = total || 0
  state.data = transformTableData(tableColumn,rows)
}
