...
async function asyncDetailData(){
  state.params.dictType = props.parentRow.dictType
  console.log(row.value)

  Object.assign(state.params,row.value)
}

async function onConfirm (cb) {
  try {
    await formRef.value?.validate()
    let useApi =  get(props.row, 'dictCode') ? updateDataApi : addDataApi
    const { status, msg, data } = await useApi(Object.assign(
      {},
      state.params,
      get(props.row, 'dictCode') && { dictCode: get(props.row, 'dictCode') } // 如果是编辑接口则添加id
    ))
