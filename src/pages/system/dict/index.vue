<template>
  ...
  <AddDictTypeDialog v-model="state.show" :row="state.row" @reload="onSearch" />
</template>
...
import { delTypeApi,  listTypeApi } from '@/api/modules/java.system.dict'
import { handleTree } from '../menu/utils'
  ...
  const actionMap:any = {
    ...
    'delete':async () => {
      ElMessageBox.confirm(
        ...
        .then(async () => {
          const { status,msg } = await delTypeApi(row.dictId)
...
const asyncData = async () => {
  ...
  const { status, msg, data,rows,total } = await listTypeApi  (params)
  if (!(status===200)) return ElMessage.error(msg)
  tableOptions.options.pagination.total = total || 0
  state.data = handleTree(transformTableData(tableColumn,rows),'dictType')
